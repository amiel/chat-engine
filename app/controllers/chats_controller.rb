class ChatsController < ApplicationController
  before_filter :validate_nick, :only => :create
  layout false
  ...
  def render_javascript_error
    render :text => 'Please enable javascript.', :layout => true
  end
  ...
  def validate_nick
    ...
    #   return true
    # else
    #   if params[:chat_nick].match Regexp.new(RESERVED_NICKS.collect{|n| '\b' + n + '\b'}.join('|'), 'i')
